...
export async function GET(request) {
  ...
  const { data, error } = await sb
    .from('projects')
    .select('id, name, client_name, url, context, signed_scope, current_phase, phases_completed, share_token, tokens_used, tokens_limit, cost_micro_usd, budget_micro_usd, created_at, updated_at')
...
export async function POST(request) {
  const { name, client_name, url, context, signed_scope, tokens_limit, budget_usd, password } = await request.json();
...
export async function PATCH(request) {
  const { projectId, tokens_limit, budget_usd, signed_scope, context, password } = await request.json();
  ...
  if (signed_scope !== undefined) updates.signed_scope = signed_scope;
